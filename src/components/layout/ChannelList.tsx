...
export default function ChannelList() {
  ...
  const serverChannels = channels.filter(c => (c as any).serverId === currentServer?.id);
  ...
  const uncategorized = serverChannels
    .filter(c => c.channelType !== ChannelType.Category && !(c as any).parentId)
    .sort((a, b) => ((a as any).position ?? 0) - ((b as any).position ?? 0));

  function getChildren(categoryId: number) {
    return serverChannels
      .filter(c => (c as any).parentId === categoryId && c.channelType !== ChannelType.Category)
      .sort((a, b) => ((a as any).position ?? 0) - ((b as any).position ?? 0));
  }
  ...
  async function handleSelectChannel(c: AbstractChannel) {
    if (currentChannel?.id === c.id) return;
    setCurrentChannel(c);
    joinChannel(c.id);
    connection?.invoke("JoinChannel", c.id).catch(() => {});
    localStorage.setItem("currentChannelId", String(c.id));
    try {
      const msgs = await getMessages(c.id, undefined, { headers: { Authorization: `Bearer ${token}` } });
      addMessages(msgs);
    } catch (e) {
  ...
  async function handleDeleteChannel(c: AbstractChannel) {
    ...
    try {
      await deleteChannel(currentServer.id, c.id, { headers: { Authorization: `Bearer ${token}` } });
  ...
  function buildCtxItems(channel: AbstractChannel): ContextMenuItem[] {
    ...
    if (canManage || serverOwner) {
      items.push({ label: "Create Channel Here", icon: "➕", onClick: () => setCreateChannelOpen(true) });
    }

    if (canInvite || serverOwner) {
      items.push({ label: "Copy Invite Link", icon: "🔗", onClick: () => {
        const url = currentServer?.inviteUrls?.[0];
        if (url) navigator.clipboard.writeText(url);
      }});
    }

    items.push({ label: "Copy Channel ID", icon: "🆔", onClick: () => navigator.clipboard.writeText(String(channel.id)) });

    return items;
  }

  function renderChannel(c: AbstractChannel, indent = false) {
    if (c.channelType === ChannelType.Category) return null;
        ...
        return (
          <div key={cat.id}>
            <div
              ...
              style={{
                display: "flex", alignItems: "center", padding: "12px 8px 4px",
